...
def login():
    ...
    if request.method == 'POST':
        ...
        with db.cursor() as cursor:
            cursor.execute(
                'SELECT accountId, password FROM Account WHERE username = %s', (username,)
            )
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        with db.cursor() as cursor:
            cursor.execute(
                'SELECT accountId, password, isAdmin FROM Account WHERE username = %s', (username,)
            )
            user = cursor.fetchone()
